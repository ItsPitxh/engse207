...
async function updateTaskStatus(taskId, newStatus) {
    ...
    try {
        ...
        if (index !== -1) allTasks[index] = updated.task;
        renderTasks();
    } catch (error) {
...
function createStatusButtons(taskId, currentStatus) {
    ...
    if (currentStatus !== 'IN_PROGRESS') {
        buttons.push(`<button class="btn btn-info btn-sm" onclick="updateTaskStatus(${taskId}, 'IN_PROGRESS')">In Progress</button>`);
    }
...
async function updateTaskStatus(taskId, newStatus) {
    ...
    try {
        ...
        const index = allTasks.findIndex(t => t.id === taskId);
        if (index !== -1) {
            allTasks[index].status = data.task.status;
        }
